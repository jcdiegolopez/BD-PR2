...
router.post('/cargar', async (req, res) => {
  ...
  try {
    ...
    const queryMap = {
      ...
      Persona: `LOAD CSV WITH HEADERS FROM $csvUrl AS row
        CREATE (:Persona {
          nombre: row.nombre,
          dpi: row.dpi,
      ...
      Operadora: `LOAD CSV WITH HEADERS FROM $csvUrl AS row
        CREATE (:Operadora {
          nombre: row.nombre,
          pais: row.pais,
          tipo: row.tipo,
      ...
      Dispositivo: `LOAD CSV WITH HEADERS FROM $csvUrl AS row
        CREATE (:Dispositivo {
          imei: row.imei,
          marca: row.marca,
          modelo: row.modelo,
      ...
      Llamada: `LOAD CSV WITH HEADERS FROM $csvUrl AS row
      ...
      Mensaje: `LOAD CSV WITH HEADERS FROM $csvUrl AS row
        MATCH (o:Numero {numero: row.numero_origen})
        MATCH (d:Numero {numero: row.numero_destino})
        CREATE (m:Mensaje {
      ...
      Reporte: `LOAD CSV WITH HEADERS FROM $csvUrl AS row
